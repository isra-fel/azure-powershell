...
namespace Microsoft.Azure.Commands.Common.Authentication.Config.Definitions
{
    ...
    {   
        ...
        /// <summary>
        /// When the value of this config changes, reset the authenticator builder so that
        /// it will rebuild with the correct interactive authenticator.
        /// </summary>
        protected override void AfterApplyTyped(bool enableLoginByWam)
        {
            base.AfterApplyTyped(enableLoginByWam);
            // reset authenticator
            // can't use DefaultAuthenticatorBuilder.Reset() because the config value has not been set yet.
            if (AzureSession.Instance.TryGetComponent<IAuthenticatorBuilder>(AuthenticatorBuilder.AuthenticatorBuilderKey, out var builder))
            {
                builder.Reset();
            }
